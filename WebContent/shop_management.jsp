			...
			<div class="col-sm-8" style="float: none; margin: 0 auto;">
					...
					<div class="shop-body">
						...
						<h5 class="card-body" style="margin: 0px 0px -20px 0px;">
							<form name="infomodify" action="ShopIntroModify.do" method="post">
								...
								<textarea name="introarea" class="form-control" rows="3"
									placeholder="<%=S_intro%>"
									style="resize: none; font-size: 15pt;"></textarea>
								<input type="button" class="btn btn-primary" value="가게소개 수정"
									onclick="infomodi()" style="float: right; margin-top: 10px;" />
				...
				<div class="card my-4">
					...
					int M_num, M_price;%>
						...
						<table class="table">
							<tr>
								<th style="font-size: 14px">메뉴번호</th>
								<th style="font-size: 14px">메뉴이름</th>
								<th style="font-size: 14px">메뉴가격</th>
								<th style="font-size: 14px">메뉴소개</th>
							...
							<tr>
								...
								<td><form action="MenuDelete.do" method="post" style="margin: 0px -10px;">
										<input type="hidden" id="shop_num" name="shop_num"
											value=<%=num%> /> <input type="hidden" id="menu_num"
											name="menu_num" value=<%=M_num%> /> <input type="submit"
				...
				<form action="menu_plus.jsp" name="menuplus" method="post"
					target="menuplus">
					<input type="hidden" name="shop_num" value="<%=num%>"> <input
						type="button" class="btn btn-primary" value="메뉴추가"
						onclick="plusmenu()"
					...
					<ul class="pagination justify-content-center">
						<c:forEach var="i" begin="<%=startpage%>" end="<%=endpage%>"
							...
							<li class="page-item"><a class="page-link"
								href="shop_management.do?shop_num=<%=num %>&page=${i }"><c:out
										value="${i }" /></a></li>
						</c:forEach>
					</ul>
				</div>
